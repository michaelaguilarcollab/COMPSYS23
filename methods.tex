...
The middle management layer is a representation of the functional blocks making up the OFMF. The HPC disaggregated infrastructure is represented under a single Redfish tree that includes all the fabrics and resources available to the system. The OFMF is the central repository for information used for authentication of clients, access control, and event logs.  Client requests are received to the OFMF through the Composability Layer, are forwarded to the appropriate fabric manager via a dedicated light-weight technology-specific Agent. 
