...
Traditional HPC systems are provisioned with static fixed quantities of memory, storage, accelerators, and CPU resources to execute requested computation.  This is not sufficient for today's datacenters that are running modern dynamic workloads which require a diverse set of resources and results in workloads running on systems that are not optimized for their needs.  Workloads may require hardware resources e.g. GPUs that are present in the datacenter but not on the server on which the workload is running.  Conversely, compute resources on a given server may be under utilized because they are not required by the workload running on that server.  Thus, datacenters often end up overprovisioning hardware resources to attempt to enable any workload to run on any server.   

Composable Disaggregated Infrastructure (CDI) enables servers to be dynamically composed of hardware resources in the datacenter as needed by a given workload.  The OpenFabrics Alliance in collaboration with the DMTF, SNIA, and the CXL Consortium, is developing an OpenFabrics Management Framework (OFMF) and a hardware Composability Manager.  The OFMF is a open-source Resource Manager which provides centralized resource monitoring and command control for the attached hardware resources and network fabrics, through matching network Agents. The Composability Manager enables assembling of resources to match workload requirements.  Together the OFMF and the Composability Manager provide the capability for dynamic configuration and provisioning of resources to maintain running client computations.  
