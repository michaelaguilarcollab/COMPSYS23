...
\begin{abstract}
Current HPC systems are limited in performance and architecture by the the stranding of disaggregated components. In current HPC systems, common tools do not exist to manage increasingly diverse network fabrics and component resources, such as, CPUs, hierarchical memory components, remote NVMe, and accelerators.  Due to lack of access to good Resource Management tools, current HPC systems suffer from inefficient over-design.  Current Compute Node resources are siloed.  In addition, the inability to dynamically share available resources, as they are required by running batch jobs can lead to reduced and inefficient computational performance and running job failure.  Centralized resource management can potentially mitigate, out-of-memory conditions, IO thashing, stranding of available resources, such as, CPUs, GPUs, and memories, and provide dynamic network fail-over.  It is clear that Resource Management, using a standardized interface, can enable clients to monitor, compose, and intelligently provision resources, in beneficial ways.

Traditional HPC systems are provisioned with static fixed quantities of memory, storage, accelerators, and CPU resources to execute application workloads.  This is not sufficient for today's datacenters that are running modern dynamic workloads.  Workloads running on these traditional HPC systems require a diverse set of resources; this results in workloads running on systems that are not optimized for their needs.  Workloads may require hardware resources e.g. GPUs in the datacenter but not on the server that they are running.  Cnnversely, compute resources on a given serer may be under utilized because they are not required by the workload running on that server.  Thus, datacenters often end up overprovisioning hardware resources to enable any workload to run on any server.   

The inability to dynamically share available resources, as they are required by running batch jobs can lead to reduced and inefficient computational performance and running job failure.  Centralized resource management can potentially mitigate, out-of-memory conditions, IO thashing, stranding of available resources, such as, CPUs, GPUs, and memories, and provide dynamic network fail-over.  It is clear that Resource Management, using a standardized interface, can enable clients to monitor, compose, and intelligently provision resources, in beneficial ways.
